...
class Pergunta07 {
  ...
  /**
   * Retorna true caso todos os nós na árvore sejam iguais ao char c
   * 
   * @param arvore: árvore binária 
   * @param c: caractere para a comparação;
   * @param pos: nó atual (utilizado para a recursividade, ao chamar 
   * a função inicialmente devemos passar o nó raíz da árvore)
   * @return true ou false
   */
  public boolean todosChar(ArvoreBinaria arvore, char c, No pos) {
    boolean retorno;

    if (pos.dir == null && pos.esq == null) {
      retorno = true;
    } else if (pos.dir.elemento != c || pos.esq.elemento != c) {
      retorno = false;
    } else {
      retorno = todosChar(arvore, c, pos.dir) && todosChar(arvore, c, pos.esq);
    }
