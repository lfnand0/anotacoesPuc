...
class Lista {
    ...
    public String pesquisaBinaria(Object nome) {
        // Função auxiliar
        this.ordenar();
        return pesquisaBinaria(array, nome, 0, n - 1);
    }
    
    public String pesquisaBinaria(Object[] arr, Object key, int esq, int dir) {
        // Realiza a pesquisa binária do valor "key" na lista
        int pivo = esq  + ((dir - esq) / 2);
        String retorno = "NÃO";
        
        if (!(dir < esq)) {
            compareCounter++;
            if (((String)key).compareTo((String)arr[pivo]) == 0) {
                compareCounter++;
                retorno = "SIM";
            } else if (((String)key).compareTo((String)arr[pivo]) < 0) {
                compareCounter += 2;
                retorno = pesquisaBinaria(arr, key, esq, pivo - 1);
            } else {
                compareCounter += 2;
                retorno = pesquisaBinaria(arr, key, pivo + 1, dir);
            } 
        }
        compareCounter++;
...
class TP02Q04 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numEntrada; i++) {
            ...
            MyIO.println(lista.pesquisaBinaria(entrada[i]));
