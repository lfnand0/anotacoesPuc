...
void empilhar (Serie item) {
  Celula *tmp = novaCelula(item);
...
int main()
{
  ...
  for (int i = 0; i < numEntrada + 1; i++) {
    ...
    char prefixo[MAX_LINE_SIZE];
    strcpy(prefixo, PREFIXO);

    int pos = 0;
    for (int i = 2; i < MAX_LINE_SIZE && line[i] != '\0'; i++) {
      aux[pos++] = line[i];
    }
    pos--;

    while (pos > 0 && (aux[pos] == ' ' || aux[pos] == '\n')) {
      aux[pos] = '\0';
      pos--;
    }
    
    if (line[0] == 'I') {
      char *html = ler_html(strcat(prefixo, aux));
      ler_serie(&serie, html);
      ...
      char serieNome[MAX_LINE_SIZE];
      strcpy(serieNome, desempilhar().nome);
